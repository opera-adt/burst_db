...
def create_2d_geojsons(
    in_file: str = f"opera-s1-disp-{VERSION_EXTRA_CLEAN}.gpkg",
) -> list[Path]:
    ...
    db_file_2d = f"opera-s1-disp-{VERSION_EXTRA_CLEAN}-2d.gpkg"
...
"""
    ...
    for query, name in [
        ...
        json_file = f"{name}-{VERSION_EXTRA_CLEAN}.geojson"
