...
# Bash command used to concatenate all zipped CSVs of the burst rows:
#
...
def create_denormalized_db(
    input_csv_glob: str = "burst_20*.csv",
    filename: str = "all_bursts.duckdb",
    export_base_name: str = "all_bursts_with_is_land",
):
    """Combine the denormalized OPERA burst ID/frame ID tables with historical bursts.

    Exports results as parquet, CSV, and sqlite.
    Requires `duckdb`.
    """
    import duckdb
    ...
    query = f"""CREATE TABLE bursts AS
    ...
    query = f"COPY bursts_denormalized TO '{export_base_name}.csv' (FORMAT CSV);"
    print(query)
    con.sql(query)
    query = (
        f"COPY bursts_denormalized TO '{export_base_name}.parquet' (FORMAT PARQUET);"
    )
    print(query)
    con.sql(query)
    con.close()
    ...
    """
    con2 = sqlite3.connect(f"{export_base_name}.sqlite3")
    con2.execute(query)
...
def get_bursts_over_north_america(
    frame_to_burst_json: str = "~/dev/opera-s1-disp-frame-to-burst.json.zip",
):
    frame_to_burst = read_zipped_json(frame_to_burst_json)
    opera_frames = [
